...
public class ListaDobleG<Tipovar extends Comparable> implements Serializable {
    ...
    public void insertarDomin(char izquierda, char derecha) {
        NodoDobleGP<Tipovar> p = new NodoDobleGP<>();
        p.setLetraDer(derecha);
        p.setLetraIzq(izquierda);
        if (vacia()) {
            Inicio = p;
            Fin = p;
            cant++;
            ...
            if (izquierda == Fin.getLetraDer()) {
                Fin.setEnlaceDer(p);
                p.setEnlaceIzq(Fin);
                Fin = p;
                cant++;
            } else {
                if (derecha == Inicio.getLetraIzq()) {
                    p.setEnlaceDer(Inicio);
                    Inicio.setEnlaceIzq(p);
                    Inicio = p;
                    cant++;
